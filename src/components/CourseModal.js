  ...
  const handleMissionStart = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error('Course selection failed:', errorData);
        alert(errorData.error || '코스 선택에 실패했습니다.');
        return;
      }
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              {course.highlights.map((hl, idx) => (
                <div key={idx} className="bg-indigo-50 rounded-lg p-3 text-sm text-gray-700">
                  {hl}
                </div>
              ))}
            </div>
